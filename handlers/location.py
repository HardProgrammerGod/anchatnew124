...
# Статичный словарь регионов и стран — хранится в памяти процесса Render, разгружая БД
LOCATIONS = {
    "europe_east": {
        "text": "Eastern Europe",
        "countries": {"EU_E_UA": "Ukraine", "EU_E_PL": "Poland", "EU_E_RO": "Romania", "EU_E_CZ": "Czechia", "EU_E_HU": "Hungary"}
    },
    "europe_west": {
        "text": "Western & North Europe",
        "countries": {"EU_W_UK": "United Kingdom", "EU_W_FR": "France", "EU_W_DE": "Germany", "EU_W_IT": "Italy", "EU_W_ES": "Spain", "EU_W_NL": "Netherlands"}
    },
    "asia": {
        "text": "Asia & Central Asia",
        "countries": {"AS_JP": "Japan", "AS_KR": "South Korea", "AS_CN": "China", "AS_IN": "India", "AS_KZ": "Kazakhstan", "AS_UZ": "Uzbekistan"}
    },
    "middle_east": {
        "text": "Middle East",
        "countries": {"ME_IL": "Israel", "ME_TR": "Turkey", "ME_AE": "UAE", "ME_SA": "Saudi Arabia"}
    },
    "americas": {
        "text": "Americas",
        "countries": {"AM_US": "USA", "AM_CA": "Canada", "AM_MX": "Mexico", "AM_BR": "Brazil", "AM_AR": "Argentina"}
    }
}

# Вспомогательная функция для быстрого поиска красивого названия страны по её коду
...
async def select_country(callback: CallbackQuery):
    ...
    # Защита от потенциального KeyError при перезапусках бота
...
# --- СОХРАНЕНИЕ С ВЫВОДОМ КРАСИВОГО ИМЕНИ ---
...
async def save_location(callback: CallbackQuery):
    ...
    # Сохраняем компактный код (6-8 байт) в Supabase
    db.update_user_location(callback.from_user.id, loc_code)
    
    # Получаем красивое название страны на лету без запроса к БД
