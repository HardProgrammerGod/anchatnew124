...
# Импортируем функцию, чтобы превратить "EU_W_DE" в "Germany"
...
def get_premium_promo_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⭐ Get Premium", callback_data="buy_premium")],
        [InlineKeyboardButton(text="🚫 Close Notice", callback_data="close_notice")]
    ])

# --- НАЧАТЬ ПОИСК (ИСПРАВЛЕНО ОТОБРАЖЕНИЕ СТРАН) ---
...
async def search_partner(callback: CallbackQuery, bot: Bot):
    ...
    if session and session.get("user_two"):
        ...
        # Декодируем технические коды в красивые названия стран на лету
        p1_country_name = get_country_name_by_code(p1_user['location_code'])
        p2_country_name = get_country_name_by_code(p2_user['location_code'])
        
        m1 = f"🎉 <b>Partner found!</b>\nLocation: <b>{p2_country_name}</b>\n\nSay hello!"
        m2 = f"🎉 <b>Partner found!</b>\nLocation: <b>{p1_country_name}</b>\n\nSay hello!"
...
async def block_media(message: Message):
    ...
    await message.answer(
        ...
        "⭐ Invite <b>1 friend</b> via the Premium section to unlock visual status updates and expand capabilities.",
        parse_mode="HTML",
...
async def handle_text_chat(message: Message, bot: Bot):
    ...
    if LINK_PATTERN.search(message.text):
        return await message.answer(
            ...
            "⭐ Invite <b>1 friend</b> via the Premium section to unlock advanced profile features.",
            parse_mode="HTML",
            reply_markup=get_premium_promo_markup()
        )
